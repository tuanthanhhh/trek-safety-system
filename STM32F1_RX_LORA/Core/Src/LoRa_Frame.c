...
{
	lora.id_device   = 1;
	lora.type_data   = TYPE_DEFAULT;
	lora.batt        = 98 + (*send_count % 5);
	lora.gps.lat     = 12.2458f + (*send_count % 5);
	lora.gps.lng     = 109.1942f + (*send_count % 5);
	lora.sensor.temp = 28 + (*send_count % 5);
	lora.sensor.humi = 70 + (*send_count % 5);

	uint16_t crc_payload_len = sizeof(lora_frame_t) - sizeof(lora.crc);
    lora.crc = calculate_crc16((uint8_t*)&lora, crc_payload_len);
}
...
uint8_t LoRa_Read_Frame(lora_frame_t *p_frame)
{
    ...
    uint8_t rx_len = LoRa_receive(&myLoRa, rx_buffer, RX_BUFFER_SIZE - 1);
    ...
    if (rx_len == sizeof(lora_frame_t))
    {
        ...
        memcpy(p_frame, rx_buffer, sizeof(lora_frame_t));
